            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {plan.accommodation.options.map((option, idx) => (
                <div key={idx} className="group bg-slate-50/50 rounded-[2.5rem] p-8 border border-transparent hover:border-indigo-100 hover:bg-white transition-all hover:shadow-2xl">
                  ...
                  <h4 className="text-lg font-bold text-slate-900 mb-1">{option.name}</h4>
                  <p className="text-xs font-bold text-pink-500 uppercase tracking-widest mb-4">{option.type}</p>
                  
                  <div className="flex items-center gap-3 mb-6">
                    <span className="px-3 py-1 bg-amber-100 text-amber-700 rounded-lg text-xs font-bold">★ {option.rating}</span>
                    <span className="text-[10px] font-bold text-slate-300 uppercase">{option.reviewCount}</span>
                  </div>
                  
                  <p className="text-sm text-slate-600 font-medium leading-relaxed italic mb-8">"{option.highlight}"</p>
        ...
        <div className="lg:col-span-4 space-y-10">
          <section className="bg-slate-900 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group">
            ...
            <div className="space-y-10 relative z-10">
              <div>
                <p className="text-5xl font-bold tracking-tighter">₹{(plan.costBreakdown.total / 1000).toFixed(1)}k</p>
              ...
              <div className="space-y-6 pt-10 border-t border-white/10">
                {[
                  ...
                  { label: 'Others', val: (plan.costBreakdown.food + plan.costBreakdown.activities), color: 'bg-amber-400' }
                ].map((item, idx) => (
                  <div key={idx}>
                    ...
                    <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div className={`${item.color} h-full rounded-full transition-all duration-1000 delay-300`} style={{ width: `${(item.val / plan.costBreakdown.total) * 100}%` }}></div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-4">
              {plan.travelOptions.map((opt, i) => (
                <div key={i} className="p-5 bg-slate-50 rounded-2xl border border-transparent hover:border-indigo-100 transition-all">
                  <p className="font-bold text-slate-800 mb-1">{opt.mode}</p>
            ...
            {plan.localTips.map((tip, i) => (
              <div key={i} className="bg-white/10 backdrop-blur-md p-8 rounded-[2rem] border border-white/10 hover:bg-white/20 transition-all">
                <div className="flex items-center gap-4 mb-4">
                  <span className="px-3 py-1 bg-white text-indigo-600 text-[10px] font-bold rounded-lg uppercase tracking-wider">{tip.category}</span>
                </div>
          ...
          {plan.itinerary.map((day, i) => (
            <div key={i} className="group flex flex-col md:flex-row bg-white rounded-[3.5rem] shadow-sm border border-slate-100 overflow-hidden hover:shadow-2xl transition-all">
              <div className="md:w-64 gradient-bg p-12 flex flex-col items-center justify-center text-white text-center">
                <span className="text-6xl font-bold tracking-tighter">0{day.day}</span>
              ...
              <div className="flex-1 p-10 md:p-14 space-y-10">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <h4 className="text-2xl font-bold text-slate-900 leading-tight">{day.title}</h4>
                  <span className="px-5 py-2 bg-slate-50 text-slate-500 text-xs font-bold rounded-xl border border-slate-100">Est. Spend: ₹{day.estimatedCost}</span>
                </div>
